...
bool see(MOVE move, int threshold) {
    ...
    // if the difference betwwen the target piece value and the attacker value is bigger than the threshold,
    // it's a good enough capture, and we return a positive outcome (meaning it's a good capture)
    if (value >= 0)
        return true;
